...
$sqlRoutes = "SELECT buses.id AS bus_id, buses.bus_number, buses.total_seats, routes.start_location, routes.end_location, 
            routes.departure_time, routes.arrival_time, routes.id AS route_id 
            FROM buses 
            JOIN routes ON buses.id = routes.bus_id
            WHERE buses.company_id = ?";
    ...
    <table border="1">
        ...
        <?php while ($route = $resultRoutes->fetch_assoc()) { ?>
            <tr>
                ...
                <td><a href="reserve.php?bus_id=<?= urlencode($route['bus_id']) ?>">Book Ticket</a></td>
            </tr>
        <?php } ?>
    </table>
</body>
